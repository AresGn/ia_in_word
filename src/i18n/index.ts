import { createI18n } from 'vue-i18n'

const messages = {
  en: {
    apiLabel: 'API',
    apiPlaceholder: 'Please choose API',
    replyLanguageLabel: 'Reply in',
    replyLanguagePlaceholder: 'Reply in',
    localLanguageLabel: 'UI Language',
    localLanguagePlaceholder: 'UI Language',
    officialAPIKeyLabel: 'API Key',
    officialAPIKeyPlaceholder: 'API key',
    officialBasePathLabel: 'Forward domain',
    officialBasePathPlaceholder: 'Forward domain',
    officialCustomModelLabel: 'Custom Model',
    officialCustomModelPlaceholder: 'Please input model',
    officialTemperatureLabel: 'Temperature',
    officialMaxTokensLabel: 'Max tokens',
    officialModelSelectLabel: 'Model',
    officialModelSelectPlaceholder: 'Please select model',
    azureAPIKeyLabel: 'API Key',
    azureAPIKeyPlaceholder: 'API key',
    azureAPIEndpointLabel: 'endpoint',
    azureAPIEndpointPlaceholder: 'endpoint',
    azureDeploymentNameLabel: 'Deploy name',
    azureDeploymentNamePlaceholder: 'Deploy name',
    azureTemperatureLabel: 'Temperature',
    azureMaxTokensLabel: 'Max tokens',
    geminiAPIKeyLabel: 'Gemini API Key',
    geminiAPIKeyPlaceholder: 'Gemini API key',
    geminiModelSelectLabel: 'Model',
    geminiModelSelectPlaceholder: 'Please select model',
    translate: 'Trans',
    summary: 'Sum',
    polish: 'Polish',
    grammar: 'Grammar',
    academic: 'Academic',
    settings: 'Settings',
    result: 'Result',
    confirm: 'Confirm',
    cancel: 'Cancel',
    settingOpen: 'Open',
    settingClose: 'Close',
    settingOllamaEndpoint: 'endpoint',
    settingProxy: 'Proxy',
    settingClickToShow: 'Click to set',
    settingEnableProxy: 'Enable proxy',
    settingProxyHost: 'Host',
    settingProxyPort: 'Port',
    settingProxyUsername: 'Username',
    settingProxyPassword: 'Password',
    settingProxyProtocol: 'Protocol',
    homeSystem: 'Character',
    homeSystemDescription: 'Please input the system prompt',
    addSystemPrompt: 'Add system prompt',
    addSystemPromptAlias: 'Alias',
    addSystemPromptAliasDescription: 'Please input the system prompt alias',
    addSystemPromptDescription: 'Please input the system prompt',
    removeSystemPrompt: 'Remove system prompt',
    homePrompt: 'Prompt',
    homePromptDescription: 'Please input the prompt',
    addPrompt: 'Add prompt',
    addPromptAlias: 'Alias',
    addPromptAliasDescription: 'Please input the prompt alias',
    addPromptDescription: 'Please input the prompt',
    removePrompt: 'Remove prompt',
    continue: 'Continue',
    start: 'Start',
    insertType: 'Insert',
    insertTypePlaceholder: 'Select insert type',
    replace: 'Replace',
    append: 'Append',
    newLine: 'New line',
    NoAction: 'No action',
    emptyAPIKey: 'Go to settings to set API key',
    backToHome: 'Back to home',
    togetherAPIKeyLabel: 'Together API Key',
    togetherAPIKeyPlaceholder: 'Together API key',
    togetherModelSelectLabel: 'Model',
    togetherModelSelectPlaceholder: 'Please select model',
    openrouterAPIKeyLabel: 'OpenRouter API Key',
    openrouterAPIKeyPlaceholder: 'OpenRouter API key',
    openrouterModelSelectLabel: 'Model',
    openrouterModelSelectPlaceholder: 'Please select model',
    apiKeySuccess: 'API Key saved successfully!',
    apiKeyError: 'Please enter a valid API key for the selected API'
  },
  'zh-cn': {
    apiLabel: 'API',
    apiPlaceholder: '请选择API',
    replyLanguageLabel: '回复语言',
    replyLanguagePlaceholder: '回复语言',
    localLanguageLabel: 'UI语言',
    localLanguagePlaceholder: 'UI语言',
    officialAPIKeyLabel: 'API 密钥',
    officialAPIKeyPlaceholder: 'API 密钥',
    officialBasePathLabel: '转发域名',
    officialBasePathPlaceholder: '转发域名',
    officialCustomModelLabel: '自定义模型',
    officialCustomModelPlaceholder: '请输入模型',
    officialTemperatureLabel: 'temperature',
    officialMaxTokensLabel: 'max tokens',
    officialModelSelectLabel: '选择模型',
    officialModelSelectPlaceholder: '请选择模型',
    azureAPIKeyLabel: 'API 密钥',
    azureAPIKeyPlaceholder: 'API 密钥',
    azureAPIEndpointLabel: 'endpoint',
    azureAPIEndpointPlaceholder: 'endpoint',
    azureDeploymentNameLabel: '部署名',
    azureDeploymentNamePlaceholder: '部署名',
    azureTemperatureLabel: 'temperature',
    azureMaxTokensLabel: 'max tokens',
    geminiAPIKeyLabel: 'Gemini API 密钥',
    geminiAPIKeyPlaceholder: 'Gemini API 密钥',
    geminiModelSelectLabel: '选择模型',
    geminiModelSelectPlaceholder: '请选择模型',
    translate: '翻译',
    summary: '摘要',
    polish: '润色',
    grammar: '语法',
    academic: '学术',
    settings: '设置',
    result: '结果',
    confirm: '确认',
    cancel: '取消',
    settingOpen: '开',
    settingClose: '关',
    settingOllamaEndpoint: 'endpoint',
    settingProxy: '代理',
    settingClickToShow: '点击设置',
    settingEnableProxy: '启用代理',
    settingProxyHost: '代理主机地址',
    settingProxyPort: '代理端口',
    settingProxyUsername: '代理用户名',
    settingProxyPassword: '代理密码',
    settingProxyProtocol: '代理协议',
    homeSystem: '角色设定',
    homeSystemDescription: '请输入角色设定提示',
    addSystemPrompt: '添加角色设定提示',
    addSystemPromptAlias: '别名',
    addSystemPromptAliasDescription: '请输入角色设定提示别名',
    addSystemPromptDescription: '请输入角色设定提示',
    removeSystemPrompt: '移除角色设定',
    homePrompt: '提示',
    homePromptDescription: '请输入提示',
    addPrompt: '添加提示',
    addPromptAlias: '别名',
    addPromptAliasDescription: '请输入提示别名',
    addPromptDescription: '请输入提示',
    removePrompt: '移除提示',
    continue: '继续',
    start: '开始',
    insertType: '插入方式',
    insertTypePlaceholder: '选择插入方式',
    replace: '替换',
    append: '追加',
    newLine: '新行',
    NoAction: '不操作',
    emptyAPIKey: '请先去设置API key',
    backToHome: '返回主页',
    togetherAPIKeyLabel: 'Together API 密钥',
    togetherAPIKeyPlaceholder: 'Together API 密钥',
    togetherModelSelectLabel: '选择模型',
    togetherModelSelectPlaceholder: '请选择模型',
    openrouterAPIKeyLabel: 'OpenRouter API 密钥',
    openrouterAPIKeyPlaceholder: 'OpenRouter API 密钥',
    openrouterModelSelectLabel: '选择模型',
    openrouterModelSelectPlaceholder: '请选择模型',
    apiKeySuccess: 'API 密钥保存成功！',
    apiKeyError: '请为所选 API 输入有效的 API 密钥'
  },
  fr: {
    apiLabel: 'API',
    apiPlaceholder: 'Veuillez choisir une API',
    replyLanguageLabel: 'Répondre en',
    replyLanguagePlaceholder: 'Répondre en',
    localLanguageLabel: "Langue de l'interface",
    localLanguagePlaceholder: "Langue de l'interface",
    officialAPIKeyLabel: 'Clé API',
    officialAPIKeyPlaceholder: 'Clé API',
    officialBasePathLabel: 'Domaine de transfert',
    officialBasePathPlaceholder: 'Domaine de transfert',
    officialCustomModelLabel: 'Modèle personnalisé',
    officialCustomModelPlaceholder: 'Veuillez saisir le modèle',
    officialTemperatureLabel: 'Température',
    officialMaxTokensLabel: 'Tokens maximum',
    officialModelSelectLabel: 'Modèle',
    officialModelSelectPlaceholder: 'Veuillez sélectionner un modèle',
    azureAPIKeyLabel: 'Clé API',
    azureAPIKeyPlaceholder: 'Clé API',
    azureAPIEndpointLabel: 'Point de terminaison',
    azureAPIEndpointPlaceholder: 'Point de terminaison',
    azureDeploymentNameLabel: 'Nom de déploiement',
    azureDeploymentNamePlaceholder: 'Nom de déploiement',
    azureTemperatureLabel: 'Température',
    azureMaxTokensLabel: 'Tokens maximum',
    geminiAPIKeyLabel: 'Clé API Gemini',
    geminiAPIKeyPlaceholder: 'Clé API Gemini',
    geminiModelSelectLabel: 'Modèle',
    geminiModelSelectPlaceholder: 'Veuillez sélectionner un modèle',
    translate: 'Traduire',
    summary: 'Résumer',
    polish: 'Améliorer',
    grammar: 'Grammaire',
    academic: 'Académique',
    settings: 'Paramètres',
    result: 'Résultat',
    confirm: 'Confirmer',
    cancel: 'Annuler',
    settingOpen: 'Ouvert',
    settingClose: 'Fermé',
    settingOllamaEndpoint: 'Point de terminaison',
    settingProxy: 'Proxy',
    settingClickToShow: 'Cliquer pour configurer',
    settingEnableProxy: 'Activer le proxy',
    settingProxyHost: 'Hôte',
    settingProxyPort: 'Port',
    settingProxyUsername: "Nom d'utilisateur",
    settingProxyPassword: 'Mot de passe',
    settingProxyProtocol: 'Protocole',
    homeSystem: 'Personnage',
    homeSystemDescription: "Veuillez saisir l'instruction système",
    addSystemPrompt: 'Ajouter une instruction système',
    addSystemPromptAlias: 'Alias',
    addSystemPromptAliasDescription:
      "Veuillez saisir l'alias de l'instruction système",
    addSystemPromptDescription: "Veuillez saisir l'instruction système",
    removeSystemPrompt: "Supprimer l'instruction système",
    homePrompt: 'Instruction',
    homePromptDescription: "Veuillez saisir l'instruction",
    addPrompt: 'Ajouter une instruction',
    addPromptAlias: 'Alias',
    addPromptAliasDescription: "Veuillez saisir l'alias de l'instruction",
    addPromptDescription: "Veuillez saisir l'instruction",
    removePrompt: "Supprimer l'instruction",
    continue: 'Continuer',
    start: 'Démarrer',
    insertType: 'Insérer',
    insertTypePlaceholder: "Sélectionner le type d'insertion",
    replace: 'Remplacer',
    append: 'Ajouter',
    newLine: 'Nouvelle ligne',
    NoAction: 'Aucune action',
    emptyAPIKey: 'Veuillez configurer la clé API dans les paramètres',
    backToHome: "Retour à l'accueil",
    togetherAPIKeyLabel: 'Clé API Together',
    togetherAPIKeyPlaceholder: 'Clé API Together',
    togetherModelSelectLabel: 'Modèle',
    togetherModelSelectPlaceholder: 'Veuillez sélectionner un modèle',
    openrouterAPIKeyLabel: 'Clé API OpenRouter',
    openrouterAPIKeyPlaceholder: 'Clé API OpenRouter',
    openrouterModelSelectLabel: 'Modèle',
    openrouterModelSelectPlaceholder: 'Veuillez sélectionner un modèle',
    apiKeySuccess: 'Clé API enregistrée avec succès !',
    apiKeyError: "Veuillez saisir une clé API valide pour l'API sélectionnée"
  }
}

export const i18n = createI18n({
  legacy: false,
  locale: localStorage.getItem('localLanguage') || 'fr',
  fallbackLocale: 'fr',
  messages
})
